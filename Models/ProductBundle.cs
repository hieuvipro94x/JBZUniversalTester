...
public sealed class ProductBundle
{
    ...
    public string UartModelPath { get; init; } = string.Empty;
    public string UartSetupPath { get; init; } = string.Empty;
    ...
    public static ProductBundle Load(string path)
    {
        ...
        if(r.TryGetProperty("uartTtl",out var u))
        {
            if(u.TryGetProperty("model",out var m)) um=Resolve(m.GetString()??"");
            if(u.TryGetProperty("setup",out var s)) us=Resolve(s.GetString()??"");
        }
        return new ProductBundle { SourcePath=full, PartNumber=r.TryGetProperty("partNumber",out var pn)?pn.GetString()??Path.GetFileNameWithoutExtension(full):Path.GetFileNameWithoutExtension(full), D2xxThtPath=d2,UartModelPath=um,UartSetupPath=us };
    }
}
