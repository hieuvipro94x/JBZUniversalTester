...
$masterModelsPath = Join-Path $ProjectRoot 'Models\MasterModels.cs'
...
$masterModels = Get-Content $masterModelsPath -Raw -Encoding UTF8
...
Add-Check 'Version 12.10.2' ($version -match '<Version>12\.10\.2</Version>') 'Version.props = 12.10.2.'
Add-Check 'Version tag 12_10_2' ($version -match '<VersionFileTag>12_10_2</VersionFileTag>') 'EXE tag đồng bộ.'
Add-Check 'Backup TestWindow V12.10.1' (Test-Path (Join-Path $ProjectRoot 'Backup\TestWindow_V12_10_1_before_master_fault_fix.xaml')) 'Có backup XAML trước sửa.'
Add-Check 'Backup TestViewModel V12.10.1' (Test-Path (Join-Path $ProjectRoot 'Backup\TestViewModel_V12_10_1_before_master_fault_fix.cs')) 'Có backup ViewModel trước sửa.'
Add-Check 'Backup MasterModels V12.10.1' (Test-Path (Join-Path $ProjectRoot 'Backup\MasterModels_V12_10_1_before_master_fault_fix.cs')) 'Có backup Master key trước sửa.'
...
Add-Check 'Master key canonical Wrong/Short' ($masterModels -match 'ProductFaultType\.WrongWiring or ProductFaultType\.ShortCircuit[\s\S]*?return new MasterFaultKey\(ProductFaultType\.WrongWiring, a, b, 0, 0\)') 'Wrong/Short cùng cạnh vật lý dùng một canonical key.'
Add-Check 'Master key normalizes reverse pair' ($masterModels -match 'NormalizePair\(ref a, ref b\)') 'A-B và B-A không tăng count hai lần.'
Add-Check 'Master key RelatedIos fallback' ($masterModels -match 'fault\.RelatedIos[\s\S]*?ElementAtOrDefault') 'Fault thiếu Expected/Actual vẫn có key theo RelatedIos.'
Add-Check 'Duplicate detail can upgrade' ($vm -match 'ShouldReplaceMasterFaultDetail\(existing, fault\)') 'Duplicate không tăng count nhưng có thể nâng detail hiển thị.'
Add-Check 'Master display rows rebuild' ($vm -match 'RebuildMasterFaultDisplayRows\(\)') 'Legacy MasterFaults đồng bộ snapshot sau replace.'
Add-Check 'Master grid deterministic order' ($vm -match '_masterDetectedFaultDetails\.Values[\s\S]*?OrderBy\(fault => FaultTypeCatalog\.Priority\(fault\.Type\)\)') 'FaultGrid Master có thứ tự ổn định.'
Add-Check 'DataGrid header bold' ($xaml -match 'TargetType="DataGridColumnHeader"[\s\S]*?FontWeight" Value="Bold"') 'Header cột rõ, đậm.'
